...
def repeating_key_xor( buf1:bytes, keyString:str ) -> bytes:
    return bytes([b1 ^ b2 for b1, b2 in zip(buf1, RepeatingKeyGenerator( bytes(keyString,'utf-8')) )])
...
def calculate_xor_keysize( ciphertext:bytes, max_key_size:int=MAX_KEY_SIZE, debug:bool=False ) -> Tuple[int, Dict[int, float]]:
    ...
    if debug:
        print( "Calculating XOR Key Size between 1 and " + str(max_key_size)  + " bytes.")
...
def detect_diff_start( buf1:bytes, buf2:bytes ):
    ...
    while( idx < lb1 and idx < lb2 and buf1[idx] == buf2[idx] ):
        idx += 1
